...
const translationsLink =
  ...
  '[https://files.skinledger.com/counterstrike/csgo_english.txt](https://files.skinledger.com/counterstrike/csgo_english.txt)';
...
async function getTranslations(items) {
  ...
  try {
    ...
    const returnValue = await axios.get(translationsLink).then((response) => {
      ...
      const data = [response.data](http://response.data);
...
class items {
  ...
  inventoryConverter(inventoryResult, isCasket = false) {
    ...
    for (const [key, value] of Object.entries(inventoryResult)) {
      ...
      if (musicIndexBytes) {
        ...
        [value.music](http://value.music)_index = musicIndexBytes.readUInt32LE(0);
  ...
  getGraffitiKitName(graffitiID) {
    ...
    for (const [key, value] of Object.entries(
      ...
      if ([value.id](http://value.id) == graffitiID) {
